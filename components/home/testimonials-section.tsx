            ...
            <div key={i} className="bg-white p-4 sm:p-6 rounded-lg shadow-md">
              <div className="flex items-center gap-1 mb-3 sm:mb-4">
                {Array(5)
                  .fill(0)
                  .map((_, j) => (
                    <Star key={j} size={14} className="sm:w-4 sm:h-4 fill-amber-400 text-amber-400" />
                  ))}
              </div>
              <p className="text-sm sm:text-base text-gray-600 mb-3 sm:mb-4 italic">"{t.text}"</p>
              <div className="flex items-center gap-3">
                <div className="h-10 w-10 sm:h-12 sm:w-12 rounded-full bg-gray-200 flex items-center justify-center text-gray-500 font-bold text-sm sm:text-base">
                  {t.name[0]}
                </div>
                <div>
                  <p className="font-semibold text-sm sm:text-base">{t.name}</p>
                  <p className="text-xs sm:text-sm text-gray-500">{t.country}</p>
                </div>
              </div>
            </div>
          ))}
